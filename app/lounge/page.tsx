...
export const metadata = {
  title: "Lounge",
  description: "Supreme-only collector feed — short posts, follows, and replies.",
  robots: { index: false, follow: false },
}

/** Supreme Lounge — Twitter-like social feed for Supreme accounts. */
export default async function LoungePage() {
  const auth = await requireUser()
  if (!auth.ok) redirect(`/sign-in?next=${encodeURIComponent("/lounge")}`)
      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-col px-4 py-8 sm:px-6">
        <header className="mb-8 flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <CollecToolsBrand href="/" size="lg" subtitle="Lounge · Supreme preview" />
            <h1 className="mt-5 text-balance text-2xl font-bold tracking-tight text-foreground sm:text-3xl">
              Lounge
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              A private collector feed for Supreme — post, follow, like, and reply. Not indexed and
              not on the public hub.
            </p>
          </div>
