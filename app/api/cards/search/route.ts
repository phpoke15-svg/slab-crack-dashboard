...
import { getCachedSearchPayload, setCachedSearchPayload } from "@/lib/cache/search-cache"
...
export async function GET(request: Request) {
  ...
  const cached = await getCachedSearchPayload(q)
  if (cached?.results) {
    return NextResponse.json(
      {
        results: cached.results as CardSearchHit[],
        catalogReady: true,
  ...
  try {
    ...
    await setCachedSearchPayload(q, {
      results,
